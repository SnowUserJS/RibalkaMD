module.exports = {
 ...
 noteFeed: async (parent, { cursor }, { models }) => {
   // Жестко кодируем лимит в десять элементов
   const limit = 10;
   // Устанавливаем значение false для hasNextPage по умолчанию
   let hasNextPage = false;
   // Если курсор передан не будет то по умолчанию запрос будет пуст
   // В таком случае из Базы Данных будут извлечины последние заметки.
   let cursorQuery = {};
   // Если курсор задан, запрос будет искать заметки со значением ObjectId меньше этого курсора
   if (cursor) {
      cursorQuery = { _id: { $lt: cursor }};
   }
   // Находим в Базе Данных limit +1 заметок, сортируя их от старых к новым.
   ...
   // как true и обрезаем заметки до лимита.
